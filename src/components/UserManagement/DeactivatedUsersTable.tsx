import { UserCheck, Trash2, ChevronLeft, ChevronRight, UserX } from 'lucide-react';
  ...
  if (users?.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="text-sm font-medium text-gray-900 mb-1">No Deactivated Users</h3>
    ...
    <div className="space-y-3">
      ...
      <div className="md:hidden space-y-3">
        {users && users.map((user) => (
          <div
            key={user._id}
            className="bg-white border border-gray-100 rounded-xl p-3 space-y-2 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.1),0_1px_3px_-1px_rgba(0,0,0,0.06)] opacity-75"
          >
              ...
              <div className="flex items-center gap-1.5 pt-2 border-t border-gray-100">
                {canManageUsers && (
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 text-xs text-green-600 bg-green-50 hover:bg-green-100 rounded transition-colors"
                  >
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              <th className="text-left py-2 px-3 text-xs font-semibold text-gray-600">Last Role</th>
              <th className="text-left py-2 px-3 text-xs font-semibold text-gray-600">Status</th>
              <th className="text-left py-2 px-3 text-xs font-semibold text-gray-600 hidden xl:table-cell">Deactivated</th>
            ...
            {users && users.map((user) => (
              <tr key={user._id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="py-2 px-3 hidden xl:table-cell">
                  <p className="text-xs text-gray-500">{formatDate((user as any).updatedAt)}</p>
                </td>
                <td className="py-2 px-3">

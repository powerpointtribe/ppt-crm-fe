...
import {
  Rocket, Layers, LayoutDashboard, Users, UserPlus, LayoutGrid,
  Building2, Calendar, ClipboardList, Wallet, Mail, BookOpen,
  GraduationCap, Package, Shield, Settings, User, Link2, HelpCircle,
  Search, ArrowUp, ChevronDown, Menu, X, Download, FileText
} from 'lucide-react'

const SECTIONS = [
  { id: 'getting-started', title: 'Getting Started', icon: Rocket, color: '#3b82f6' },
  { id: 'system-overview', title: 'System Overview', icon: Layers, color: '#3b82f6' },
  { id: 'dashboard', title: 'Dashboard', icon: LayoutDashboard, color: '#3b82f6' },
  { id: 'members', title: 'Members', icon: Users, color: '#6366f1' },
  { id: 'first-timers', title: 'First Timers', icon: UserPlus, color: '#6366f1' },
  { id: 'groups', title: 'Groups', icon: LayoutGrid, color: '#8b5cf6' },
  { id: 'campuses', title: 'Campuses', icon: Building2, color: '#8b5cf6' },
  { id: 'events', title: 'Events', icon: Calendar, color: '#8b5cf6' },
  { id: 'service-reports', title: 'Service Reports', icon: ClipboardList, color: '#8b5cf6' },
  { id: 'finance', title: 'Finance', icon: Wallet, color: '#10b981' },
  { id: 'bulk-email', title: 'Bulk Email', icon: Mail, color: '#f59e0b', wip: true },
  { id: 'library', title: 'Library', icon: BookOpen, color: '#f59e0b', wip: true },
  { id: 'workers-training', title: 'Workers Training', icon: GraduationCap, color: '#f59e0b', wip: true },
  { id: 'inventory', title: 'Inventory', icon: Package, color: '#f59e0b', wip: true },
  { id: 'roles', title: 'Roles & Permissions', icon: Shield, color: '#64748b' },
  { id: 'administration', title: 'Administration', icon: Settings, color: '#64748b' },
  { id: 'personal-settings', title: 'Personal Settings', icon: User, color: '#64748b' },
  { id: 'public-forms', title: 'Public Forms', icon: Link2, color: '#3b82f6' },
  { id: 'faq', title: 'FAQ', icon: HelpCircle, color: '#64748b' },
]

function FAQItem({ question, children }: { question: string; children: React.ReactNode }) {
  const [open, setOpen] = useState(false)
  return (
    <div className={`docs-faq-item ${open ? 'open' : ''}`} onClick={() => setOpen(!open)} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && setOpen(!open)}>
      <div className="docs-faq-q">
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="docs-faq-a">{children}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
      ...
      <div className="docs-layout">
        ...
        <aside className="docs-sidebar">
          ...
          <div className="docs-search-wrap">
            ...
            <input ref={searchInputRef} type="text" placeholder="Search... ⌘K" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="docs-search-input" />
          </div>
          <nav className="docs-sidebar-nav">
            {filteredSections.map((s) => {
              const Icon = s.icon
              return (
                <button key={s.id} onClick={() => scrollToSection(s.id)} className={`docs-sidebar-item ${activeSection === s.id ? 'active' : ''}`}>
                  <span className="docs-sidebar-indicator" style={{ background: s.color }} />
                  <Icon size={14} style={{ color: s.color, flexShrink: 0 }} />
                  <span className="docs-sidebar-label">{s.title}</span>
                  {s.wip && <span className="docs-wip-dot" />}
                </button>
              )
            })}
          </nav>
        ...
        <main className="docs-main">
          ...
          <div className="docs-cover">
            <div className="docs-cover-pattern" />
            <div className="docs-cover-glow" />
            <div className="docs-cover-content">
              <span className="docs-version-badge">v1.0 &mdash; March 2026</span>
              <h1 className="docs-cover-title">Church Management System</h1>
              <p className="docs-cover-subtitle">Complete Feature Guide for Team Members</p>
              <div className="docs-cover-divider" />
              <p className="docs-cover-org">PowerPoint Tribe</p>
              <div className="docs-cover-actions">
                <button className="docs-download-btn" onClick={downloadPDF} disabled={downloading}>
                  {downloading ? <span className="docs-spinner" /> : <Download size={18} />}
                  {downloading ? 'Generating PDF...' : 'Download as PDF'}
                </button>
              </div>
              {statusText && <p className="docs-status-text">{statusText}</p>}
            </div>
          </div>

          {/* Quick Jump Pills */}
          <div className="docs-quick-jump">
            {SECTIONS.filter(s => !s.wip).slice(0, 10).map((s) => {
              const Icon = s.icon
              return (
                <button key={s.id} onClick={() => scrollToSection(s.id)} className="docs-pill">
                  <Icon size={13} />
                  <span>{s.title}</span>
                </button>
              )
          ...
          <div ref={contentRef}>
            ...
            {/* 1. Getting Started */}
            <div className="docs-section" id="getting-started" style={{ borderLeftColor: '#3b82f6' }}>
              <h2><Rocket size={22} className="docs-section-icon" style={{ color: '#3b82f6' }} />1. Getting Started</h2>
            ...
            {/* 2. System Overview */}
            <div className="docs-section" id="system-overview" style={{ borderLeftColor: '#3b82f6' }}>
              <h2><Layers size={22} className="docs-section-icon" style={{ color: '#3b82f6' }} />2. System Overview</h2>
            ...
            {/* 3. Dashboard */}
            <div className="docs-section" id="dashboard" style={{ borderLeftColor: '#3b82f6' }}>
              <h2><LayoutDashboard size={22} className="docs-section-icon" style={{ color: '#3b82f6' }} />3. Dashboard</h2>
            ...
            {/* 4. Members */}
            <div className="docs-section" id="members" style={{ borderLeftColor: '#6366f1' }}>
              <h2><Users size={22} className="docs-section-icon" style={{ color: '#6366f1' }} />4. Members Management</h2>
            ...
            {/* 5. First Timers */}
            <div className="docs-section" id="first-timers" style={{ borderLeftColor: '#6366f1' }}>
              <h2><UserPlus size={22} className="docs-section-icon" style={{ color: '#6366f1' }} />5. First Timers (Visitor Management)</h2>
              ...
              <ol className="docs-steps">
                ...
                <li>Fill in the form:
                  <ul>
                    ...
                    <li><strong>How they heard about the church</strong> &mdash; Friend, social media, outreach, walk-by, etc.</li>
                    ...
                    <li><strong>Interests and prayer requests</strong> (optional)</li>
                  </ul>
                </li>
              ...
              <p>Each time you contact a first timer, you should record what happened:</p>
              <ol className="docs-steps">
                ...
                <li>Record the details:
                  <ul>
                    ...
                    <li><strong>Notes</strong> &mdash; Key details from the conversation</li>
              ...
              <ul>
                ...
                <li>Who has made the most calls</li>
              ...
              <p>Go to <strong>First Timers &gt; Messages</strong> to create and manage <strong>message templates</strong> for follow-up communication. These are pre-written messages that can be reused when reaching out to first timers via SMS or WhatsApp.</p>
            ...
            {/* 6. Groups */}
            <div className="docs-section" id="groups" style={{ borderLeftColor: '#8b5cf6' }}>
              <h2><LayoutGrid size={22} className="docs-section-icon" style={{ color: '#8b5cf6' }} />6. Groups (Districts, Units &amp; Ministries)</h2>
              ...
              <table>
                ...
                <tbody>
                  <tr><td><strong>District</strong></td><td>An organizational division of the church. Districts help organize the church into manageable sections.</td><td>District Pastor</td></tr>
                  <tr><td><strong>Unit</strong></td><td>A service arm of the church where functional duties are carried out. Units are where the actual work happens &mdash; for example, LDI, Beckoners, Chimes and Chills, Waves, Buzz, etc.</td><td>Unit Head</td></tr>
                  <tr><td><strong>Ministry</strong></td><td>A group of related units brought together under an overarching vision. For example, the <em>Missions Ministry</em> contains units like Tribesheart, R199, and LDI.</td><td>Ministry Director</td></tr>
                </tbody>
              </table>
              ...
              <div className="docs-info-box">
                <strong>Example:</strong> The <em>Missions Ministry</em> (led by a Ministry Director) contains three units: <em>Tribesheart</em>, <em>R199</em>, and <em>LDI</em>. Each unit has its own Unit Head who manages the day-to-day operations, while the Ministry Director provides overarching direction for all three.
              </div>
            ...
            {/* 7. Campuses */}
            <div className="docs-section" id="campuses" style={{ borderLeftColor: '#8b5cf6' }}>
              <h2><Building2 size={22} className="docs-section-icon" style={{ color: '#8b5cf6' }} />7. Campuses</h2>
              <p>The PowerPoint Tribe is a multi-campus church with <strong>multiple physical locations</strong>. The Campuses module allows us to manage each location separately. Each campus has its own members, service times, and contact information.</p>
            ...
            {/* 8. Events */}
            <div className="docs-section" id="events" style={{ borderLeftColor: '#8b5cf6' }}>
              <h2><Calendar size={22} className="docs-section-icon" style={{ color: '#8b5cf6' }} />8. Events</h2>
            ...
            {/* 9. Service Reports */}
            <div className="docs-section" id="service-reports" style={{ borderLeftColor: '#8b5cf6' }}>
              <h2><ClipboardList size={22} className="docs-section-icon" style={{ color: '#8b5cf6' }} />9. Service Reports (Attendance Tracking)</h2>
            ...
            {/* 10. Finance */}
            <div className="docs-section" id="finance" style={{ borderLeftColor: '#10b981' }}>
              <h2><Wallet size={22} className="docs-section-icon" style={{ color: '#10b981' }} />10. Finance &amp; Requisitions</h2>
              <p>The Finance module manages <strong>financial requisitions</strong> &mdash; formal requests for funds. Every requisition goes through an <strong>approval workflow</strong> to ensure proper authorization before any money is released.</p>
              ...
              <table>
                ...
                <tbody>
                  ...
                  <tr><td><strong>Approved / Rejected</strong></td><td>An authorized approver reviews the request and either approves or rejects it (with a reason for rejection).</td><td>Users with approval permission</td></tr>
              ...
              <ol className="docs-steps">
                ...
                <li>Click <strong>"Approve"</strong> to authorize the expense, or <strong>"Reject"</strong> (you must provide a reason for rejection).</li>
              </ol>
              ...
              <ul>
                <li><strong>Expense categories</strong> &mdash; Define the types of expenses that can be selected.</li>
            ...
            {/* 11. Bulk Email */}
            <div className="docs-section" id="bulk-email" style={{ borderLeftColor: '#f59e0b' }}>
              <h2><Mail size={22} className="docs-section-icon" style={{ color: '#f59e0b' }} />11. Bulk Email &amp; Communication</h2>
            ...
            {/* 12. Library */}
            <div className="docs-section" id="library" style={{ borderLeftColor: '#f59e0b' }}>
              <h2><BookOpen size={22} className="docs-section-icon" style={{ color: '#f59e0b' }} />12. Library Management</h2>
              <div className="docs-note"><strong>Work in Progress:</strong> This module is available in the system but is still being developed.</div>
              ...
              <p>Go to <strong>Library &gt; Overdue</strong> to see all books past their return date, along with the borrower's contact information and how many days overdue.</p>
            </div>

            {/* 13. Workers Training */}
            <div className="docs-section" id="workers-training" style={{ borderLeftColor: '#f59e0b' }}>
              <h2><GraduationCap size={22} className="docs-section-icon" style={{ color: '#f59e0b' }} />13. Workers Training</h2>
              <div className="docs-note"><strong>Work in Progress:</strong> This module is available in the system but is still being developed.</div>
            ...
            {/* 14. Inventory */}
            <div className="docs-section" id="inventory" style={{ borderLeftColor: '#f59e0b' }}>
              <h2><Package size={22} className="docs-section-icon" style={{ color: '#f59e0b' }} />14. Inventory Management</h2>
              <div className="docs-note"><strong>Work in Progress:</strong> This module is available in the system but is still being developed.</div>
            ...
            {/* 15. Roles */}
            <div className="docs-section" id="roles" style={{ borderLeftColor: '#64748b' }}>
              <h2><Shield size={22} className="docs-section-icon" style={{ color: '#64748b' }} />15. User Roles &amp; Permissions</h2>
              <p>The system uses <strong>role-based access control</strong> to determine what each user can see and do. Every user is assigned a role, and each role has a specific set of permissions.</p>
              ...
              <table>
                ...
                <tbody>
                  <tr><td><strong>Super Admin / Senior Pastor</strong></td><td>The highest level of access. Full access to everything &mdash; all members, all campuses, all settings, user management, role management, system configuration, and church-wide oversight.</td></tr>
                  ...
                  <tr><td><strong>Member</strong></td><td>Basic access. Can view their own profile, access first timers assigned to them, and use features specifically assigned to them.</td></tr>
                </tbody>
              </table>
              <h3>Custom Roles</h3>
              <p>In addition to the built-in roles, administrators can create <strong>custom roles</strong> with specific permissions:</p>
              <ul>
              ...
              <p>Permissions are organized by module. Each module has specific actions that can be individually enabled or disabled:</p>
              <ul>
            ...
            {/* 16. Administration */}
            <div className="docs-section" id="administration" style={{ borderLeftColor: '#64748b' }}>
              <h2><Settings size={22} className="docs-section-icon" style={{ color: '#64748b' }} />16. Administration</h2>
              ...
              <ol className="docs-steps">
                ...
                <li>Optionally, link them to an <strong>existing member record</strong>.</li>
              ...
              <p>If someone should no longer have access, you can <strong>deactivate</strong> their account. This prevents them from logging in but preserves all their historical data and audit trail.</p>
              <h3>Roles Management</h3>
              <p>Go to <strong>Administration &gt; Roles</strong> to view, create, edit, or delete roles.</p>
              <h3>Audit Logs</h3>
              ...
              <p>Go to <strong>Audit Logs &gt; Statistics</strong> for visual charts showing activity patterns.</p>
              <h3>Bulk Upload</h3>
              <p>Go to <strong>Administration &gt; Bulk Upload</strong> to import large numbers of members via CSV files.</p>
              <h3>Admin Settings</h3>
              <p>Go to <strong>Administration &gt; Admin Settings</strong> to configure system-wide settings:</p>
              <div className="docs-feature-grid">
                <div className="docs-feature-box"><h4>General Settings</h4><p>Church name, address, phone, email, and website.</p></div>
                <div className="docs-feature-box"><h4>Notification Settings</h4><p>Enable or disable email and SMS notifications, configure follow-up reminders.</p></div>
                <div className="docs-feature-box"><h4>Security Settings</h4><p>Password expiry policy, session timeout, two-factor authentication, max login attempts.</p></div>
                <div className="docs-feature-box"><h4>System Settings</h4><p>Timezone, date format, backup frequency, and maintenance mode.</p></div>
              </div>
            </div>

            {/* 17. Personal Settings */}
            <div className="docs-section" id="personal-settings" style={{ borderLeftColor: '#64748b' }}>
              <h2><User size={22} className="docs-section-icon" style={{ color: '#64748b' }} />17. Personal Settings</h2>
              <p>Go to <strong>My Settings</strong> (at the bottom of the sidebar) to manage your <strong>personal preferences</strong>. These settings only affect your own experience.</p>
              <ul>
            ...
            {/* 18. Public Forms */}
            <div className="docs-section" id="public-forms" style={{ borderLeftColor: '#3b82f6' }}>
              <h2><Link2 size={22} className="docs-section-icon" style={{ color: '#3b82f6' }} />18. Public Forms (Shareable Links)</h2>
              <p>The system includes several <strong>public-facing pages</strong> that can be accessed by <strong>anyone without logging in</strong>. These are designed to be shared widely &mdash; via WhatsApp, social media, email, printed flyers, or QR codes.</p>
              <div className="docs-info-box">
                <strong>How to use:</strong> Replace <code>your-app-url</code> with your church's actual app web address. For campus-specific forms, replace <code>campus-name</code> with the campus slug.
              </div>
              ...
              <p>Share this with first-time visitors so they can register their own details. The information they submit automatically creates a first-timer record in the system.</p>
              <h3>Event Registration Form</h3>
              <p><strong>Link:</strong> <code>your-app-url/event-registration/event-name</code></p>
              <p>Each event automatically gets its own unique public registration page. Share this link for people to sign up for the event.</p>
              ...
              <p>Allows external stakeholders or team members without system access to submit financial requisitions through a public form.</p>
            ...
            {/* 19. FAQ */}
            <div className="docs-section" id="faq" style={{ borderLeftColor: '#64748b' }}>
              <h2><HelpCircle size={22} className="docs-section-icon" style={{ color: '#64748b' }} />19. Frequently Asked Questions</h2>

              <h3>General</h3>
              <FAQItem question="I can't see a feature mentioned in this guide. Why?">
                Your role may not have permission to access that feature. The system only shows features that are relevant to your assigned role. If you believe you should have access to a specific feature, contact your administrator to review your role permissions.
              </FAQItem>
              <FAQItem question="I forgot my password. What do I do?">
                Click "Forgot Password?" on the login page. Enter your email address and check your inbox (including spam/junk folder) for a reset link. Click the link to create a new password.
              </FAQItem>
              <FAQItem question="Can I use this on my phone?">
                Yes. The system is fully mobile-responsive and works on any device with a web browser &mdash; phones, tablets, laptops, and desktop computers. No app download is required.
              </FAQItem>
              <FAQItem question="Who can I contact for technical support?">
                Contact your system administrator (typically the Super Admin or Admin). They can help with login issues, role changes, and technical questions.
              </FAQItem>

              <h3>Members</h3>
              <FAQItem question="How do I find a specific member?">
                Go to the Members page and use the search bar at the top. You can search by name, email, or phone number. You can also use the filter options to narrow results by district, unit, membership status, or gender.
              </FAQItem>
              <FAQItem question="How do I move a member to a different unit or district?">
                Open the member's profile, click "Edit", and change their unit or district assignment from the Church Info section. Click Save to apply.
              </FAQItem>
              <FAQItem question="Can I export the members list?">
                Yes. On the Members page, look for the Export button. This will download the current list (with any active filters applied) as a CSV file that you can open in Excel or Google Sheets.
              </FAQItem>

              <h3>First Timers</h3>
              <FAQItem question="How do I know which first timers I need to follow up with?">
                Go to <strong>"My Assigned First Timers"</strong> in the sidebar. This shows only the visitors assigned to you. First timers with a "New" or "Assigned" status are the ones still waiting for contact.
              </FAQItem>
              <FAQItem question="What happens when I mark a first timer as 'Ready for Integration'?">
                This signals to administrators that the visitor has been successfully followed up and is ready to become a full church member. An admin can then click "Integrate to Member" to convert their visitor record into a complete member profile &mdash; all existing information is carried over automatically.
              </FAQItem>
              <FAQItem question="Can visitors register themselves?">
                Yes. You can share a public visitor registration link (see <a href="#public-forms" onClick={(e) => e.stopPropagation()}>Section 18</a>). Visitors who fill out this form will automatically appear in the First Timers list, ready for assignment.
              </FAQItem>

              <h3>Finance</h3>
              <FAQItem question="Who approves my requisition?">
                Requisitions are approved by users who have the "Approve Requisition" permission. This is typically assigned to Admins, Pastors, or designated Finance Officers.
              </FAQItem>
              <FAQItem question="Can I edit a requisition after submitting it?">
                No. Once submitted, it is locked. This maintains the integrity of the approval process. You can only edit requisitions in "Draft" status. If changes are needed after submission, contact an admin who can reject it so you can create a corrected version.
              </FAQItem>
              <FAQItem question="How do I know when my requisition is approved or rejected?">
                You'll receive a notification. You can also check the status at any time from the requisitions list page.
              </FAQItem>

              <h3>Groups</h3>
              <FAQItem question="What's the difference between a District, Unit, and Ministry?">
                A <strong>District</strong> is an organizational division led by a District Pastor. A <strong>Unit</strong> is a service arm where functional duties are carried out (like LDI, Beckoners, Chimes and Chills, Waves, or Buzz), led by a Unit Head. A <strong>Ministry</strong> is a group of related units under an overarching vision (like the Missions Ministry containing Tribesheart, R199, and LDI), led by a Ministry Director.
              </FAQItem>
              <FAQItem question="Can a member belong to multiple groups?">
                A member is assigned to one district and one unit. Since units are organized into ministries, their ministry is determined by their unit assignment. Each member also belongs to one campus.
              </FAQItem>
            </div>
        ...
        {/* Mobile TOC Button */}
        <button className="docs-mobile-toc-btn" onClick={() => setShowMobileToc(true)}>
        ...
        <AnimatePresence>
          {showMobileToc && (
            <>
              <motion.div className="docs-drawer-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowMobileToc(false)} />
              <motion.div className="docs-drawer" initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 300 }}>
                <div className="docs-drawer-header">
                  <span>Contents</span>
                  <button onClick={() => setShowMobileToc(false)}><X size={20} /></button>
                </div>
                <nav className="docs-drawer-nav">
                  {SECTIONS.map((s) => {
                    const Icon = s.icon
                    return (
                      <button key={s.id} onClick={() => scrollToSection(s.id)} className={`docs-drawer-item ${activeSection === s.id ? 'active' : ''}`}>
        ...
        <AnimatePresence>
          {showBackToTop && (
            <motion.button className="docs-back-to-top" onClick={scrollToTop} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}>
              <ArrowUp size={18} />
            </motion.button>
          )}
        </AnimatePresence>
...
const docsStyles = `
  ...
  .docs-layout * { box-sizing: border-box; }
  ...
  .docs-sidebar-nav::-webkit-scrollbar { width: 4px; }
  .docs-sidebar-nav::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 4px; }
  .docs-sidebar-item {
    display: flex;
    align-items: center;
    gap: 10px;
  ...
  .docs-sidebar-item:hover { background: #f1f5f9; color: #1e293b; }
  .docs-sidebar-item.active { background: #eef2ff; color: #4338ca; font-weight: 600; }
  .docs-sidebar-indicator {
    position: absolute;
    left: 0;
    top: 50%;
    transform: translateY(-50%);
  ...
  .docs-sidebar-item.active .docs-sidebar-indicator { height: 20px; }
  .docs-sidebar-label { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .docs-wip-dot {
    width: 6px;
    height: 6px;
    border-radius: 50%;
  ...
  .docs-cover {
    ...
    margin-bottom: 32px;
    background: linear-gradient(160deg, #0f0a2e 0%, #1a1145 25%, #2d1b69 50%, #1e3a8a 100%);
    color: #ffffff;
    border-radius: 20px;
    overflow: hidden;
  }
  ...
  .docs-cover-content { position: relative; z-index: 1; }
  .docs-version-badge {
  ...
  .docs-cover-subtitle {
    font-size: 18px;
    color: rgba(196,207,255,0.85) !important;
  ...
  .docs-cover-org {
    font-size: 14px;
    color: rgba(196,207,255,0.6) !important;
    margin: 0 0 32px !important;
    font-weight: 500;
    letter-spacing: 2px;
    text-transform: uppercase;
  }
  .docs-cover-actions { display: flex; justify-content: center; gap: 12px; }
  .docs-download-btn {
    display: inline-flex;
    align-items: center;
    gap: 8px;
  ...
  .docs-download-btn:hover { background: rgba(255,255,255,0.18); border-color: rgba(255,255,255,0.3); transform: translateY(-1px); box-shadow: 0 8px 24px rgba(0,0,0,0.2); }
  .docs-download-btn:disabled { opacity: 0.6; cursor: not-allowed; transform: none; box-shadow: none; }
  .docs-spinner {
    display: inline-block;
    width: 18px;
    height: 18px;
  ...
  .docs-status-text { font-size: 13px; color: rgba(196,207,255,0.6) !important; margin-top: 14px !important; }
  ...
  .docs-pill:hover { border-color: #6366f1; color: #4338ca; background: #eef2ff; }
  ...
  .docs-toc-title { font-size: 20px; margin: 0 0 16px; color: #1e293b; border: none; font-weight: 700; }
  .docs-toc-list { list-style: none; counter-reset: toc-counter; padding: 0; margin: 0; columns: 2; column-gap: 24px; }
  .docs-toc-list li { counter-increment: toc-counter; margin-bottom: 4px; break-inside: avoid; }
  .docs-toc-list li a {
  ...
  .docs-toc-list li a:hover { background: #f1f5f9; color: #1e293b; }
  ...
  .docs-wip { color: #f59e0b; font-size: 10px; margin-left: 6px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
  ...
  .docs-section-icon { vertical-align: text-bottom; margin-right: 10px; }
  ...
  .docs-layout h3 { font-size: 16px; color: #1e293b; margin: 24px 0 10px; font-weight: 650; page-break-after: avoid; }
  .docs-layout h4 { font-size: 14.5px; color: #334155; margin: 18px 0 6px; font-weight: 600; page-break-after: avoid; }
  .docs-layout p { margin-bottom: 12px; color: #475569; }
  .docs-layout ul, .docs-layout ol { margin: 8px 0 16px 20px; color: #475569; }
  .docs-layout li { margin-bottom: 5px; }
  .docs-layout li strong { color: #1e293b; }
  .docs-layout ul ul, .docs-layout ol ul { margin-top: 4px; margin-bottom: 4px; }
  ...
  .docs-layout th { background: #f8fafc; color: #1e293b; padding: 10px 14px; text-align: left; font-weight: 600; border-bottom: 2px solid #e2e8f0; }
  .docs-layout td { padding: 10px 14px; border-bottom: 1px solid #f1f5f9; vertical-align: top; }
  .docs-layout tr:last-child td { border-bottom: none; }
  .docs-layout tr:nth-child(even) { background: #fafbfc; }

  /* Info Boxes */
  .docs-note { background: #fffbeb; border-left: 4px solid #f59e0b; padding: 14px 18px; border-radius: 0 8px 8px 0; margin: 14px 0 20px; font-size: 13.5px; }
  .docs-note strong { color: #b45309; }
  .docs-tip { background: #f0fdf4; border-left: 4px solid #22c55e; padding: 14px 18px; border-radius: 0 8px 8px 0; margin: 14px 0 20px; font-size: 13.5px; }
  .docs-info-box { background: #eff6ff; border-left: 4px solid #3b82f6; padding: 14px 18px; border-radius: 0 8px 8px 0; margin: 14px 0 20px; font-size: 13.5px; }
  .docs-info-box strong { color: #1d4ed8; }
  ...
  .docs-workflow-step { background: #eef2ff; color: #4338ca; padding: 8px 16px; border-radius: 20px; font-weight: 600; font-size: 12.5px; white-space: nowrap; }
  .docs-workflow-arrow { color: #94a3b8; font-size: 18px; }
  .docs-workflow-step.active { background: #4338ca; color: white; }
  .docs-workflow-step.warning { background: #fffbeb; color: #b45309; border: 1.5px solid #f59e0b; }

  /* Diagram Containers */
  .docs-diagram { background: #fafbfc; border-radius: 10px; padding: 20px; margin: 16px 0 24px; border: 1px solid #f1f5f9; text-align: center; }
  .docs-diagram-title { font-weight: 700; font-size: 13px; color: #64748b; margin-bottom: 14px; text-transform: uppercase; letter-spacing: 0.5px; }
  ...
  .docs-org-node { background: #f8fafc; border: 2px solid #6366f1; border-radius: 8px; padding: 10px 20px; font-weight: 600; font-size: 13px; color: #4338ca; text-align: center; min-width: 120px; }
  .docs-org-node.primary { background: #4338ca; color: white; border-color: #4338ca; }
  .docs-org-node.secondary { background: #6366f1; color: white; border-color: #6366f1; }
  .docs-org-node.tertiary { background: #22c55e; color: white; border-color: #22c55e; }
  .docs-org-connector { font-size: 18px; color: #cbd5e1; }
  .docs-org-node .docs-org-label { font-size: 11px; color: #94a3b8; margin-top: 2px; font-weight: 400; }
  .docs-org-node.primary .docs-org-label,
  .docs-org-node.secondary .docs-org-label,
  .docs-org-node.tertiary .docs-org-label { color: rgba(255,255,255,0.7); }

  /* Feature Grid */
  .docs-feature-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin: 14px 0 20px; }
  .docs-feature-box { background: #f8fafc; border-radius: 8px; padding: 14px; border: 1px solid #f1f5f9; transition: border-color 0.15s; }
  .docs-feature-box:hover { border-color: #c7d2fe; }
  .docs-feature-box h4 { margin: 0 0 4px; font-size: 14px; color: #1e293b; }
  .docs-feature-box p { margin: 0; font-size: 12.5px; color: #64748b; line-height: 1.5; }

  /* Code */
  .docs-layout code { background: #f1f5f9; padding: 2px 7px; border-radius: 4px; font-size: 12.5px; color: #6366f1; font-family: 'SF Mono', 'Fira Code', monospace; }

  /* Steps */
  .docs-steps { counter-reset: step-counter; list-style: none; margin-left: 0; padding-left: 0; }
  .docs-steps li { counter-increment: step-counter; padding-left: 34px; position: relative; margin-bottom: 10px; }
  ...
  .docs-system-overview { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin: 14px 0; }
  .docs-module-card { background: #f8fafc; border-radius: 8px; padding: 12px; text-align: center; border: 1px solid #e2e8f0; transition: border-color 0.15s; }
  .docs-module-card:hover { border-color: #c7d2fe; }
  .docs-module-card.wip { background: #fffbeb; border-color: #fde68a; }
  .docs-module-card h4 { margin: 0 0 3px; font-size: 12.5px; color: #1e293b; }
  .docs-module-card p { margin: 0; font-size: 11px; color: #64748b; line-height: 1.4; }
  .docs-wip-badge { display: inline-block; background: #f59e0b; color: white; font-size: 9px; padding: 2px 6px; border-radius: 4px; font-weight: 700; margin-top: 4px; }

  /* Nav Preview */
  .docs-nav-preview { background: #0f172a; color: #e2e8f0; border-radius: 10px; padding: 18px 22px; font-size: 13px; line-height: 2.1; display: inline-block; text-align: left; min-width: 280px; }
  .docs-nav-group { color: #64748b; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; font-weight: 700; margin-top: 8px; }
  .docs-nav-item { padding-left: 12px; }
  .docs-nav-item.sub { padding-left: 30px; color: #94a3b8; font-size: 12px; }
  ...
  .docs-faq-item:hover { border-color: #c7d2fe; }
  .docs-faq-item.open { border-color: #6366f1; background: #fafaff; }
  .docs-faq-q {
    display: flex;
    align-items: center;
    gap: 10px;
  ...
  .docs-faq-item.open .docs-faq-chevron { transform: rotate(180deg); color: #6366f1; }
  .docs-faq-a { color: #475569; padding: 10px 0 2px 26px; font-size: 13.5px; line-height: 1.6; }
  .docs-faq-a a { color: #6366f1; }

  /* Footer */
  .docs-footer { text-align: center; padding: 28px; color: #94a3b8; font-size: 13px; border-top: 1px solid #e2e8f0; margin-top: 40px; }
  ...
  .docs-drawer-header button:hover { background: #f1f5f9; }
  .docs-drawer-nav { padding: 12px 16px; }
  .docs-drawer-item {
    display: flex;
    align-items: center;
    gap: 12px;
  ...
  .docs-drawer-item:hover { background: #f1f5f9; }
  .docs-drawer-item.active { background: #eef2ff; color: #4338ca; font-weight: 600; }
  ...
  .docs-back-to-top:hover { background: #f1f5f9; color: #1e293b; }
  ...
  @media print {
    ...
    .docs-quick-jump { display: none !important; }
    .docs-mobile-toc-btn { display: none !important; }
    .docs-back-to-top { display: none !important; }
    .docs-section { box-shadow: none; border: 1px solid #ddd; transform: none !important; }
    .docs-layout h3, .docs-layout h4 { page-break-after: avoid; }
    ...
    .docs-layout table { page-break-inside: avoid; }
    .docs-layout tr { page-break-inside: avoid; }
    .docs-layout p { orphans: 3; widows: 3; }
  }

  /* Mobile */
  @media (max-width: 1024px) {
    .docs-sidebar { display: none; }
    .docs-main { margin-left: 0; padding: 20px 16px 80px; }
    .docs-mobile-toc-btn { display: flex; }
    .docs-back-to-top { bottom: 84px; }
    .docs-cover { padding: 52px 20px 48px; }
    .docs-cover-title { font-size: 28px !important; }
    .docs-section { padding: 22px 20px; }
    .docs-toc { padding: 22px 20px; }
    .docs-toc-list { columns: 1; }
    ...
    .docs-quick-jump { flex-wrap: nowrap; overflow-x: auto; -webkit-overflow-scrolling: touch; padding-bottom: 6px; }
    .docs-quick-jump::-webkit-scrollbar { display: none; }
  }
`
